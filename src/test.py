...
parser.add_argument('--path_model', type=str,
                    default='./resources/pretrained/tsrn_segment_8x.pth', help='path for pretrained models.')
parser.add_argument('--path_data', type=str,
                    default='./resources/images', help='path of test images.')
parser.add_argument('--path_output', type=str, default='./output/images',
                    help='output path for images generated.')
...
sr_net.load_state_dict(torch.load(
    opt.path_model, map_location=lambda storage, loc: storage))
if cuda:
    sr_net = sr_net.cuda()
sr_net.eval()
...
for i, image in enumerate(images):
    ...
    bicubic_img = imresize(
        img, (img.shape[0]*upsample, img.shape[1]*upsample), interp='bicubic')

    img_t = img_transform(img)
    img_t = to_variable(img_t.view(
        1, img_t.size(0), img_t.size(1), img_t.size(2)))
    bicubic_t = img_transform(bicubic_img)
    bicubic_t = to_variable(bicubic_t.view(
        1, bicubic_t.size(0), bicubic_t.size(1), bicubic_t.size(2)))
    print('======> Super Resolution Predictions are being made')
    data_out = sr_net(img_t)
    data_out = data_out + bicubic_t
    torchvision.utils.save_image((data_out.data).clamp(
        0, 1), os.path.join(output_path, name))
